...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    """这是一个实现了正弦位置编码的 PyTorch 模块 `SinusoidalPositionalEmbedding`。位置编码用于为输入序列中的每个位置生成不同的可学习的位置向量，以帮助模型更好地理解序列中的顺序信息。

这个模块的主要功能包括：

- **初始化函数 (`__init__`)：**初始化模块的参数，包括位置向量的维度、填充符号索引和初始化大小。在初始化时，调用了 `get_embedding` 方法来生成初始的正弦位置编码。
  
- **生成编码 (`get_embedding`)：**这个方法根据给定的参数生成正弦位置编码。它首先计算了每个位置的正弦和余弦值，然后根据位置索引将它们连接起来，并添加了填充索引（如果有的话）。

- **前向传播函数 (`forward`)：**根据输入的序列和位置信息，返回对应的位置编码。如果输入是增量式的，表示解码过程中的单步操作，则只返回当前时间步的位置编码。如果没有给定位置信息，则根据输入
序列自动生成位置信息。

- **最大位置数函数 (`max_positions`)：**返回支持的最大位置数，这里返回一个大的数值。

这个模块的主要用途是在自然语言处理任务中为输入序列添加位置编码，如在 Transformer 模型中用于自注意力机制中。
"""
...
class DiffusionEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        return emb
    """这是一个用于扩散步骤的嵌入层 `DiffusionEmbedding`，它用于将输入向量转换为嵌入向量。这种嵌入通常用于向模型提供关于输入的一些附加信息，以帮助模型更好地理解输入数据。

这个嵌入层的功能如下：

- **初始化函数 (`__init__`)：** 初始化嵌入层的参数，主要是嵌入向量的维度 `d_denoiser`。

- **前向传播函数 (`forward`)：** 接收一个输入张量 `x`，并返回一个经过处理的嵌入张量。在这个实现中，它首先计算了一些正弦和余弦值，并将它们连接起来以形成嵌入向量。这个嵌入向量的维度是 `d_denoiser` 的
一半，通过调整 `emb` 的值，可以控制嵌入向量的频率范围和幅度。最后返回的嵌入向量是输入张量 `x` 与这些正弦和余弦函数的乘积的连接。

这种嵌入层通常用于深度学习模型中，作为模型的一部分，用于将输入数据映射到一个更具有信息丰富性的表示形式，以便模型能够更好地学习任务。在这个特定的实现中，嵌入向量的频率和幅度是根据输入数据动态计算的，以
便更好地适应不同的输入。
"""
